...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $stmt = $pdo->prepare('INSERT INTO echipe (nume_echipa, capitan, divizie_id, data_inscriere) VALUES (:nume, :capitan, :divizie, NOW())');
        $stmt->execute([
    ...
    if ($action === 'update') {
        ...
        $stmt = $pdo->prepare('UPDATE echipe SET nume_echipa = :nume, capitan = :capitan, divizie_id = :divizie WHERE id_echipa = :id');
        $stmt->execute([
            ':nume'    => $payload['nume_echipa'],
            ':capitan' => $payload['capitan'],
            ':divizie' => (int)$payload['divizie_id'],
                ...
                <div class="card-body">
                    <form method="POST" action="echipe.php">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="capitan" name="capitan" value="<?= htmlspecialchars($teamToEdit['capitan'] ?? '') ?>" required>
                        </div>
                        <div class="mb-3">
